class Game {
    constructor() {
        ...
        this.shield = {
            ...
            lastUsed: 0,
            duration: 0,
            maxDuration: 3000, // 3 secondi di durata
            particles: [],
    ...
    setupEventListeners() {
        ...
        // Gestione click su popup game over
        this.canvas.addEventListener('click', (e) => {
            ...
            // Bottone Riprova
            if (mx >= x + w/2 - 80 && mx <= x + w/2 + 80 && my >= y + 160 && my <= y + 204) {
                this.resetGame();
            }
        });
    ...
    spawnEnemy() {
        ...
        if (selectedType === 'normale') {
            enemy = {
                x: x,
                y: y,
                speed: (0.7 + Math.pow(this.wave, 1.6) * 0.15) * waveMultiplier,
                health: Math.floor((30 + Math.pow(this.wave, 2.5)) * difficultyMultiplier),
                maxHealth: Math.floor((30 + Math.pow(this.wave, 2.5)) * difficultyMultiplier),
                reward: Math.floor((10 + Math.pow(this.wave, 1.8)) * waveMultiplier),
                width: 20,
                height: 20,
                type: 'normale',
                anim: 0,
    ...
    update() {
        ...
        if (this.enemiesSpawned >= this.enemiesPerWave && this.enemies.length === 0) {
            ...
            this.showWaveReward = {
                ...
                timer: 120, // frames (2 secondi a 60fps)
                wave: this.wave - 1
            };
        ...
        for (let i = this.enemies.length - 1; i >= 0; i--) {
            ...
            if (this.shield.active && enemy.y >= this.shield.y - this.shield.height/2 && 
                ...
                // Riduci l'energia dello scudo in base alla vita del nemico
                this.shield.power = Math.max(0, this.shield.power - enemy.health * 0.5);
                
                // Se l'energia è finita, disattiva lo scudo
                if (this.shield.power <= 0) {
                    this.shield.active = false;
                    this.shield.particles = [];
                    this.shield.lastUsed = Date.now();
                    this.shield.cooldown = this.shield.maxCooldown;
                    continue;
                }
                ...
                // Crea testo fluttuante per il danno dello scudo
                this.createFloatingText(enemy.x, enemy.y - 10, "SHIELD!", 'shield');
                
                // Mostra quanto potere è rimasto
                this.createFloatingText(enemy.x, enemy.y + 10, `${Math.floor(this.shield.power)}%`, 'shield');
            ...
            if (enemy.y > this.canvas.height) {
                ...
                // Verifica se lo scudo può assorbire parte del danno
                const remainingDamage = this.absorbDamage(damage);
                
                // Crea testo fluttuante per danno assorbito se lo scudo è attivo
                if (this.shield.active && damage > remainingDamage) {
                    const absorbed = damage - remainingDamage;
                    this.createFloatingText(this.cannon.x, this.cannon.y - 40, `${absorbed} assorbito!`, 'shield');
                }
                
                // Applica il danno rimanente
    ...
    activateShield() {
        ...
        // Calcola la potenza dello scudo in base all'ondata con un bonus più aggressivo
        // Bonus più elevato nelle ondate avanzate per mantenere lo scudo competitivo
        const baseShieldPower = 30;                                   // Potenza base
        const waveMultiplier = Math.min(7, 1 + this.wave * 0.15);     // Moltiplicatore basato sull'ondata, più alto nelle ondate avanzate
        const bonusPower = Math.pow(this.wave, 1.2) * 2;              // Bonus esponenziale per ondate avanzate
        
        this.shield.power = Math.min(100, baseShieldPower * waveMultiplier + bonusPower);
        
        // Adatta la durata dello scudo in base all'ondata (ondate più avanzate = scudo più duraturo)
        const durationBonus = Math.min(4000, this.wave * 400);        // +400ms per ondata, massimo +4s
        this.shield.duration = this.shield.maxDuration + durationBonus;  // Durata base + bonus
    ...
    // Metodo per verificare se uno scudo può assorbire il danno
absorbDamage(damage) {
    // Se non c'è scudo attivo, ritorna il danno completo
    if (!this.shield.active) {
        return damage;
    }
    
    // Percentuale di assorbimento del danno (più potente nelle ondate avanzate)
    const absorptionPercent = Math.min(0.9, 0.4 + (this.shield.power / 100) * 0.5);
    
    // Calcola danno assorbito
    const absorbed = Math.floor(damage * absorptionPercent);
    ...
    render() {
        ...
        this.ctx.fillText(`Ultimate:`, 30, 150);
        ...
        if (this.shield.active) {
            ...
            const shieldOpacity = 0.3 + shieldPulse;
            
            // Calcola colore dello scudo in base alla potenza (più ondate = colore più intenso)
            const powerFactor = this.shield.power / 100;
            const r = Math.floor(34 + powerFactor * 60);
            const g = Math.floor(170 + powerFactor * 85);
            const b = Math.floor(255);
            const shieldColor = `rgba(${r}, ${g}, ${b}, ${shieldOpacity})`;
            
            // Disegna la linea principale dello scudo
            this.ctx.shadowColor = shieldColor;
            this.ctx.shadowBlur = 15 + powerFactor * 10;
            this.ctx.strokeStyle = shieldColor;
            this.ctx.lineWidth = this.shield.height + powerFactor * 3;
            ...
            for (let i = 0; i < particleCount; i++) {
                ...
                this.ctx.beginPath();
                this.ctx.arc(x, this.shield.y + yOffset, 2 + powerFactor * 2, 0, Math.PI * 2);
        ...
        for (let bullet of this.bullets) {
            ...
            // Corpo del proiettile con gradiente
            const grad = this.ctx.createRadialGradient(bullet.x, bullet.y, 0, bullet.x, bullet.y, 3);
        ...
        // Game Over popup
        if (this.showGameOver) {
            // Effetto vignetta scura su tutto lo schermo
            this.ctx.save();
            ...
            radialGradient.addColorStop(1, 'rgba(0,0,0,0.85)');
            ...
            const boxGradient = this.ctx.createLinearGradient(x, y, x, y + h);
            boxGradient.addColorStop(0, 'rgba(40,40,55,0.98)');
            boxGradient.addColorStop(1, 'rgba(25,25,35,0.98)');
            ...
            // Disegna il box con angoli arrotondati
            this.ctx.beginPath();
            this.ctx.moveTo(x + 10, y);
            this.ctx.lineTo(x + w - 10, y);
            this.ctx.quadraticCurveTo(x + w, y, x + w, y + 10);
            this.ctx.lineTo(x + w, y + h - 10);
            this.ctx.quadraticCurveTo(x + w, y + h, x + w - 10, y + h);
            this.ctx.lineTo(x + 10, y + h);
            this.ctx.quadraticCurveTo(x, y + h, x, y + h - 10);
            ...
            this.ctx.fillText('GAME OVER', x + w/2, y + 55);
            this.ctx.shadowColor = 'transparent';
            
            // Dettagli
            this.ctx.font = 'bold 22px Arial';
            this.ctx.fillStyle = '#fff';
            this.ctx.fillText(`Ondata raggiunta: ${this.showGameOver.wave}`, x + w/2, y + 100);
            this.ctx.fillText(`Nemici uccisi: ${this.showGameOver.kills}`, x + w/2, y + 135);
            
            // Bottone Riprova con effetti
            const btnX = x + w/2 - 80;
            const btnY = y + 160;
            const btnW = 160;
            const btnH = 44;
            ...
            this.ctx.lineTo(btnX + btnW - 5, btnY);
            this.ctx.quadraticCurveTo(btnX + btnW, btnY, btnX + btnW, btnY + 5);
            this.ctx.lineTo(btnX + btnW, btnY + btnH - 5);
            this.ctx.quadraticCurveTo(btnX + btnW, btnY + btnH, btnX + btnW - 5, btnY + btnH);
            this.ctx.lineTo(btnX + 5, btnY + btnH);
            this.ctx.quadraticCurveTo(btnX, btnY + btnH, btnX, btnY + btnH - 5);
            this.ctx.lineTo(btnX, btnY + 5);
            this.ctx.quadraticCurveTo(btnX, btnY, btnX + 5, btnY);
            this.ctx.closePath();
            this.ctx.fill();
            
            // Rimuovi l'ombra per il bordo
            this.ctx.shadowColor = 'transparent';
            this.ctx.strokeStyle = 'rgba(255,255,255,0.8)';
            this.ctx.lineWidth = 2;
            this.ctx.stroke();
            ...
            this.ctx.fillText('RIPROVA', x + w/2, y + 190);
            
            this.ctx.restore();
            
            // Pulse effect per attirare l'attenzione sul bottone
            const pulseTime = Date.now() * 0.002;
            const pulseSize = 1 + Math.sin(pulseTime) * 0.03;
            
            this.ctx.save();
            this.ctx.globalAlpha = 0.2 + Math.abs(Math.sin(pulseTime * 1.5)) * 0.2;
            this.ctx.translate(x + w/2, y + 182);
    ...
    resetGame() {
        ...
        // Reset cannon e relative stats
        this.cannon.damage = 20;
        ...
        if (this.ultimates) {
            this.ultimates.cooldown = 0;
            this.ultimates.lastUsed = 0;
        }
